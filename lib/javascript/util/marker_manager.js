...
class MarkerManager {
  ...
  updateMarkers(collisionsArray, taxi, bike, motorcycle, ped) {
    const collisionsObj = {};
    collisionsArray.forEach(collision => collisionsObj[collision.unique_key] = collision);
    collisionsArray
      .filter(collision => !this.markersObj[collision.unique_key])
      .forEach((newCollision) => this.createMarker(newCollision, taxi, bike, motorcycle, ped));

    Object.keys(this.markersObj)
      .filter(collisionUniqueKey => !collisionsObj[collisionUniqueKey])
      .forEach((collisionUniqueKey) => this.removeMarker(this.markersObj[collisionUniqueKey]));
  }
